...
const ADMIN_DOMAIN = "@xclusivebarber.co.za";

export function AdminLoginForm({ onComplete, onBackAction }: AdminLoginFormProps) {
  const { login } = useAuth();
  ...
  const validateEmail = (emailStr: string): boolean => {
    ...
    if (!trimmed.endsWith(ADMIN_DOMAIN)) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (profile?.full_name) {
        // Existing admin with complete profile — log in immediately
        ...
        // New admin or incomplete profile — collect name
  ...
  const handleSaveName = async () => {
    ...
    try {
      if (profileNeedsUpdate) {
        // Profile exists but had no full_name — update it
        await updateProfile(supabaseUserId, { name: name.trim() });
      } else {
        // New admin — create profile with admin role
        await createProfile({
          id: supabaseUserId,
          email: email.trim(),
          name: name.trim(),
      ...
      <AnimatePresence mode="wait">
        {step === "email" && (
          ...
          >
            <FormHeader
              title="Admin Sign In"
              subtitle={`Enter your ${ADMIN_DOMAIN} email to verify.`}
              showBack={true}
            />
        ...
        {step === "name" && (
          ...
          >
            <FormHeader
              ...
              subtitle="We'll need this to set up your admin account."
            />
